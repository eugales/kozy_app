...
import 'package:kozy_app/services/auth/sign_in.dart';
...
void main() {
  group('signIn', () {
    test("returns User and saves Auth token", () async {
      ...
      var faker = Faker();
      final token =
          'Bearer ${faker.jwt.custom(expiresIn: DateTime.now(), payload: Map.from(userData))}';

      when(prefs.setString('authorization', token))
          .thenAnswer((_) => Future<bool>.value(true));

      when(client.post(Uri.parse('http://localhost:3000/auth/sign_in'),
      ...
      final actual =
          await signIn(client, prefs, userData: jsonEncode(userData));
